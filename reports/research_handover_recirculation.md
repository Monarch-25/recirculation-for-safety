...
Shared mechanics: `α·f(s) + β·d`, destination-L2 rescaling with eps-safe division, convex/nonconvex beta resolution (never hardcoded), paper-exact 1B ramp α_t = min(t/10,1)·α, 0-based block indexing with bounds fail-fast, BOS presence assertion, opt-in activation debug tracing (norms/cosine, capped).
